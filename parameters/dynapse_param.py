...
dynapse_param = {
    ...
    'I_tau_syn_exc': 2.5 * pA,           # Synapctic time constant current, the time constant is inversely proportional to I_tau
    'I_wo_syn_exc': 50. * pA,            # Base synaptic weight current which can be scaled by the .weight parameter
    'I_g_syn_exc': 2.5 * pA,            # DPI's threshold / gain current, scaling factor for the synaptic weight (typically equal to I_tau)
    ...
    'I_tau_syn_exc2': 50. * pA,           # Synapctic time constant current, the time constant is inversely proportional to I_tau
    'I_wo_syn_exc2': 50. * pA,            # Base synaptic weight current which can be scaled by the .weight parameter
    'I_g_syn_exc2': 50. * pA,            # DPI's threshold / gain current, scaling factor for the synaptic weight (typically equal to I_tau)
    ...
    'I_g_syn_inh': 10. * pA,            # DPI's threshold / gain current, scaling factor for the synaptic weight (typically equal to I_tau)
    ...
    'I_g_syn_shunt': 10. * pA,          # DPI's threshold / gain current, scaling factor for the synaptic weight (typically equal to I_tau)
